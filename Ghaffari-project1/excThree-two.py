...
imgGaBl = cv2.GaussianBlur(img, (7, 7), 0)
# Apply some Median blur on the image
imgMb = cv2.medianBlur(imgGaBl, 5)
# Apply a bilateral filer on the image
img_bf = cv2.bilateralFilter(imgMb, 5, 80, 80)
# Use the laplace filter to detect edges
img_lp_im = cv2.Laplacian(img, cv2.CV_8U, ksize=5)
img_lp_gb = cv2.Laplacian(imgGaBl, cv2.CV_8U, ksize=5)
img_lp_mb = cv2.Laplacian(imgMb, cv2.CV_8U, ksize=5)
...
cv2.imwrite('result/2.png', cartoon_Bilateral)
